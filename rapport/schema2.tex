...
\node[boite, above of=memctrl,node distance=3cm] (ram) {RAM};
\node[ptiteboite, below right of=memctrl] (dd) {BCD};
\node[ptiteboite, right of=dd, node distance=2.5cm] (bcd) {7-seg};

\node[reg, above of=muxers,node distance=2cm] (pc) {PC};
\node[reg, below of=muxers] (b) {B};
\node[reg, left of=b,node distance=1cm] (a) {A};
\node[reg, right of=b,node distance=1cm] (vb) {VB};
\node[ptiteboite, left of=muxers] (count) {Counter};
\node[operateur, below of=b, node distance=2cm] (sub) {sub};
\node[operateur, left of=sub] (leqz) {leqz};

\draw[fil] (count) -- (muxers);
